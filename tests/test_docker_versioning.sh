...
docker run -d --name z4-versioning -p 8080:8080 -v $(pwd)/data:/app/data z4:latest /app/z4 server
...
curl -X PUT http://localhost:8080/ver-bucket

echo "Getting versioning (should be empty/disabled)..."
VER_CONFIG=$(curl -s "http://localhost:8080/ver-bucket?versioning")
...
curl -X PUT "http://localhost:8080/ver-bucket?versioning" -d '<VersioningConfiguration><Status>Enabled</Status></VersioningConfiguration>'

echo "Getting versioning (should be Enabled)..."
VER_ENABLED=$(curl -s "http://localhost:8080/ver-bucket?versioning")
...
curl -X PUT "http://localhost:8080/ver-bucket?versioning" -d '<VersioningConfiguration><Status>Suspended</Status></VersioningConfiguration>'

echo "Getting versioning (should be Suspended)..."
VER_SUSPENDED=$(curl -s "http://localhost:8080/ver-bucket?versioning")
